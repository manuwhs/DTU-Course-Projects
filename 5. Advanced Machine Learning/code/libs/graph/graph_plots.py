...
#            print Y[:,i].shape
        ...
        if (fill == 1):  ## Fill this shit !!
            fill_i = self.fill_between(x = X[(NpX-ws):],y1 = Y[(NpX-ws):,i],y2 = 0, color = colorFinal,alpha = alpha)
...
#    X =  grba.preprocess_dates(X)
    ...
    for i in range(NcY):  # We plot once for every line to plot
        ...
        if (i >= len(legend)):
            plot_i, = ax.plot([0,0],[0,0], lw = lw, alpha = alpha, 
                     color = colorFinal, zorder = self.zorder)
        else:
#            print X.shape
#            print Y[:,i].shape
            plot_i, = ax.plot([0,0],[0,0], lw = lw, alpha = alpha, color = colorFinal,
                     label = legend[i], zorder = self.zorder)
...
def fill_between(self, x, y1, y2 = 0, where = None, alpha = 1.0 , color = "#888888", *args, **kwargs):
...
#    X = ul.fnp(X)  # It kills stuff
    if (len(legend) == 0):
        plot_i= self.axes.bar(X[(NpX-ws):], Y[(NpX-ws):], width = width, align='center',
